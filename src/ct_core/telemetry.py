...
from opentelemetry.sdk.trace.export import BatchSpanProcessor, ConsoleSpanExporter, SpanExporter, SimpleSpanProcessor, SpanExportResult
    ...
    def export(self, spans):
        for span in spans:
            if span.name.startswith("trust."):
                print(f"[OTel] Captured Evidence: {span.name} | Status: {span.status.status_code}")
        return SpanExportResult.SUCCESS
    def shutdown(self):
        pass

def setup_telemetry(service_name: str, additional_exporters: Optional[List[SpanExporter]] = None):
